	...
	if ( $childs ) {
		...
		foreach ( $childs as $child ) {
			...
			if ( is_null($courseid) or !email_have_asociated_folders($USER->id) ) {
				$subfolders[] = get_record('block_email_list_folder', 'id', $child->folderchildid);
			} else {
				if ( $folder = get_record('block_email_list_folder', 'id', $child->folderchildid, 'course', $courseid) ) {
					$subfolders[] = $folder;
				} else if ( $folder = get_record('block_email_list_folder', 'id', $child->folderchildid, 'course', '0') ) {
...
function email_get_all_subfolders($folderid) {
	...
	if ( $childs ) {
		...
		foreach ( $childs as $child ) {
				$subfolders[] = get_record('block_email_list_folder', 'id', $child->folderchildid);
				if ( $morechilds = get_records('block_email_list_subfolder', 'folderparentid',  $child->folderchildid) ) {
...
function email_get_parentfolder($folderid) {
	...
	$parent = get_record('block_email_list_subfolder', 'folderchildid', $folderid);
	...
	if ( $parent ) {
		...
		while ( is_null($folder->isparenttype) ) {
			// Searching ...
			$parent = get_record('block_email_list_subfolder', 'folderchildid', $folder->id);
			$folder = email_get_folder($parent->folderparentid);
		}
...
function email_get_foldermail($mailid, $userid) {
	...
	$sql = "SELECT f.id, f.name, fm.id as foldermail
                   FROM {$CFG->prefix}block_email_list_folder f
                   INNER JOIN {$CFG->prefix}block_email_list_foldermail fm ON f.id = fm.folderid
...
function email_get_reference2foldermail($mailid, $folderid) {
	return get_record('block_email_list_foldermail', 'mailid', $mailid, 'folderid', $folderid);
...
function email_move2folder($mailid, $foldermailid, $folderidnew) {
	...
	if ( record_exists('block_email_list_folder', 'id', $folderidnew) ) {

		// Folder have exist in this new folder?
		if (! get_record('block_email_list_foldermail', 'mailid', $mailid, 'folderid', $folderidnew) ) {

			// Change folder reference to mail
			if (! set_field('block_email_list_foldermail', 'folderid', $folderidnew, 'id', $foldermailid, 'mailid', $mailid)) {
			    	return false;
			}
		} else {
			if ( ! delete_records('block_email_list_foldermail', 'id', $foldermailid) ) {
				return false;
			}
		}
	} else {
...
function email_newfolder($folder, $parentfolder) {
	...
	// Insert record
	if (! $folder->id = insert_record('block_email_list_folder', $folder)) {
		return false;
	}
	...
	if (! insert_record('block_email_list_subfolder', $subfolder)) {
		return false;
	}
...
function email_get_folders($userid, $sort='id') {
	...
	return get_records('block_email_list_folder', 'userid', $userid, $sort);
}
...
function email_get_folder($folderid) {
	...
	if ( $folder = get_record('block_email_list_folder', 'id', $folderid) ) {
...
function email_create_parents_folders($userid) {
	...
	if ( count_records('block_email_list_folder', 'userid', $userid, 'isparenttype', EMAIL_INBOX) == 0 ) {
		if (! $folders->inboxid = insert_record('block_email_list_folder', $folder)) {
			return false;
		}
	}
	...
	if ( count_records('block_email_list_folder', 'userid', $userid, 'isparenttype', EMAIL_DRAFT) == 0 ) {
		if (! $folders->trashid = insert_record('block_email_list_folder', $folder)) {
			return false;
		}
	}
	...
	if ( count_records('block_email_list_folder', 'userid', $userid, 'isparenttype', EMAIL_SENDBOX) == 0 ) {
		if (! $folders->sendboxid = insert_record('block_email_list_folder', $folder)) {
			return false;
		}
	}
	...
	if ( count_records('block_email_list_folder', 'userid', $userid, 'isparenttype', EMAIL_TRASH) == 0) {
		if (! $folders->trashid = insert_record('block_email_list_folder', $folder)) {
			return false;
		}
	}
...
function email_removefolder($folderid, $options) {
	...
	if ( get_record('block_email_list_subfolder', 'folderparentid', $folderid) ) {
	...
	// Get folder
	if ($folders =  get_records('block_email_list_folder', 'id', $folderid)) {

	    // For all folders . . .
	    foreach($folders as $folder) {

			// Before removing references to foldermail, move this mails to root folder parent.
			if ($foldermails = get_records('block_email_list_foldermail', 'folderid', $folder->id) ) {
			...
			if (! delete_records('block_email_list_subfolder', 'folderparentid', $folder->id)) {
			    	return false;
			}

			// Delete all subfolders of this
			if (! delete_records('block_email_list_subfolder', 'folderchildid', $folder->id)) {
			    	return false;
			}

			// Delete all filters of this
			if (! delete_records('block_email_list_filter', 'folderid', $folder->id)) {
			    	return false;
			}

			// Delete all foldermail references
			if (! delete_records('block_email_list_foldermail', 'folderid', $folder->id)) {
			    	return false;
			}
	    }

	    // Delete all folders
	    if (! delete_records('block_email_list_folder', 'id', $folderid)) {
		    	return false;
		}
	}
...
function email_is_subfolder($folderid) {
	return get_record('block_email_list_subfolder', 'folderchildid', $folderid);
}
...
function email_get_my_writemails($userid, $order = NULL) {
	...
	// Get my write mails
	if ($order) {
		$mails = get_records('block_email_list_mail', 'userid', $userid, $order);
	} else {
		$mails = get_records('block_email_list_mail', 'userid', $userid);
	}
	...
	if ( $options ) {
		if ( isset($options->folderid ) ) {
			...
			if ( $options->folderid != 0 ) {
				...
				$groupby = " GROUP BY m.id, m.userid, m.course, m.subject, m.timecreated, m.body";
...
function email_get_parent_folder($folder) {
	...
	if ( is_int($folder) ) {
		if ( ! $subfolder = get_record('block_email_list_subfolder', 'folderchildid', $folder) ) {
	        return false;
	    }
	} else {
		if ( ! $subfolder = get_record('block_email_list_subfolder', 'folderchildid', $folder->id) ) {
        	return false;
		}
    }

    return get_record('block_email_list_folder', 'id', $subfolder->folderparentid);
...
function email_get_root_folder($userid, $folder) {
	...
	if ( $userid > 0 and !empty($userid) ) {
		if ( $folder == EMAIL_INBOX ) {
			$rootfolder = get_record('block_email_list_folder', 'userid', $userid, 'isparenttype', EMAIL_INBOX);
			$rootfolder->name = get_string('inbox', 'block_email_list');
			return $rootfolder;
		}

		if ( $folder == EMAIL_SENDBOX ) {
			$rootfolder = get_record('block_email_list_folder', 'userid', $userid, 'isparenttype', EMAIL_SENDBOX);
			$rootfolder->name = get_string('sendbox', 'block_email_list');
			return $rootfolder;
		}

		if ( $folder == EMAIL_TRASH ) {
			$rootfolder = get_record('block_email_list_folder', 'userid', $userid, 'isparenttype', EMAIL_TRASH);
			$rootfolder->name = get_string('trash', 'block_email_list');
			return $rootfolder;
		}

		if ( $folder == EMAIL_DRAFT ) {
			$rootfolder = get_record('block_email_list_folder', 'userid', $userid, 'isparenttype', EMAIL_DRAFT);
...
function email_get_users_sent($mailid, $forreplyall=false, $writer=NULL, $type='') {
	...
	if (! $sends = get_records('block_email_list_send', 'mailid', $mailid) ) {
		return false;
	}
...
function email_get_user($mailid) {
	...
	// Get mail record
	if (! $mail = get_record('block_email_list_mail', 'id', $mailid)) {
...
function email_have_asociated_folders($userid) {
	...
	if ( $CFG->email_marriedfolders2courses ) {
		if ( $preferences = get_record('block_email_list_preference', 'userid', $userid) ) {
...
function email_is_enabled_email_list( $courseid ) {
	...
	// Get block object
	if ($emaillist = get_record('block', 'name', 'email_list') ) {

		// Block has enable in this course?
		if ( $block = get_record_sql("SELECT *
	                                   FROM {$CFG->prefix}block_instance
	                                   WHERE pagetype = '".PAGE_COURSE_VIEW."'
	                                   AND pageid = $courseid
	                                   AND blockid = $emaillist->id") ) {
			 // If it's visible
	    	 if ($block->visible) {
	    	 	$blockobject = block_instance('email_list');
	    	    if (empty($blockobject->instance->pinned)) {
