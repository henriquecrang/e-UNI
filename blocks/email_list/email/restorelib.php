 <?php  // $Id: restorelib.php,v 1.3 2009/04/01 23:07:36 tmas Exp $
/**
 * Library of functions to restore for module email
 *
 * @author Toni Mas
 * @version $Id: restorelib.php,v 1.3 2009/04/01 23:07:36 tmas Exp $
 * @package email
...
function email_restore_mail ($info, $restore) {
    ...
    /// Remap course ID
        ...
        if (!$newemailid = insert_record('block_email_list_mail', $email)) {
                $status = false;
                break;
        }
        ...
        backup_putid($restore->backup_unique_code, 'block_email_list_mail', $oldid, $newemailid);
        ...
        if (!empty($emailinfo['#']['FOLDERMAILS']['0']['#']['FOLDERMAIL'])) {
            ...
            for ($ii = 0;$ii < count($foldersmailinfo); $ii++) {
                ...
                if (!$newfolderid = backup_getid($restore->backup_unique_code, 'block_email_list_folder', $oldfolderid)) {
                    $status = false;
                    break;
                }
                ...
                if (!$newfoldermailid = insert_record('block_email_list_foldermail', $newfoldermail)) {
                    $status = false;
                    break;
                }
...
function email_folders_restore($info,$restore) {
    ...
    /// Remap coruse ID
        ...
        if ($existingfolder = get_record('block_email_list_folder', 'userid', $folder->userid, 'name', $folder->name, 'course', $folder->course)) {
            $newfolderid = $existingfolder->id;
        } else {
            if (!$newfolderid = insert_record('block_email_list_folder', $folder)) {
                $status = false;
                break;
            }
        }
        ...
        backup_putid($restore->backup_unique_code, 'block_email_list_folder', $oldid, $newfolderid);
...
//restore the actual subfolders now
    ...
    foreach($subfolders as $subfolder) {
        if ($newchildid = backup_getid($restore->backup_unique_code, 'block_email_list_folder', $subfolder->oldfolderchildid)) {
            //found the new id
        ...
        if (!$newsubid = insert_record('block_email_list_subfolder', $subfolder)) {
            $status = false;
            break;
        }
    }
...
function email_sends_restore($info, $restore) {
    ...
    /// Remap course ID
        ...
        if ($newid = backup_getid($restore->backup_unique_code, 'block_email_list_mail', $oldid)) {
        ...
        if (!$newsentid = insert_record('block_email_list_send', $sentemail)) {
            $status = false;
            break;
        }
    }
