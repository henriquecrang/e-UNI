<?php  // $Id: backuplib.php,v 1.3 2009/04/01 23:07:36 tmas Exp $
/**
 * Library of functions to backup for module email
 *
 * @author Toni Mas
 * @version $Id: backuplib.php,v 1.3 2009/04/01 23:07:36 tmas Exp $
 * @package email
...
function email_backup_instance($bf, $preferences, $courseid) {
    ...
    if ($emails = get_records('block_email_list_mail', 'course', $courseid)) {
        ...
        if ($folders = get_records_select('block_email_list_folder', $where)) {
        ...
        if ($sentemails = get_records('block_email_list_send', 'course', $courseid)) {
...
function email_backup_mail($bf,$preferences,$email) {
    ...
    if ($foldermails = get_records('block_email_list_foldermail', 'mailid', $email->id)) {
...
function email_backup_folder($bf,$preferences, $folder, $folders) {
    ...
    if (empty($folder->isparenttype)) {
        $sql = "SELECT f.*
                FROM {$CFG->prefix}block_email_list_folder f, {$CFG->prefix}block_email_list_subfolder sf
    ...
    if ($subfolders = get_records('block_email_list_subfolder', 'folderparentid', $folder->id)) {
