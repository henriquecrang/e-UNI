<?php  // $Id: participants.php,v 1.14 2009/10/19 22:33:00 tmas Exp $
/**
 * This page prints all participants or contacts who sents mail/s
 *
 * @uses $CFG
 * @author Toni Mas
 * @version $Id: participants.php,v 1.14 2009/10/19 22:33:00 tmas Exp $
 * @package email
	...
	if ( $CFG->email_old_select_participants ) {
		...
		foreach ($alpha as $ch) {
		 		...
		 		if ($ch == $firstinitial) {
		     		...
		     		echo "<a href=\"$wwwroot/blocks/email_list/email/participants.php?id=$courseid&amp;group=$selgroup&amp;perpage=$perpage&amp;search=$search&amp;roleid=$roleid&amp;lname=$lastinitial&amp;fname=$ch')\">$ch</a>";
		 		}
		 		$nch++;
		}
		echo '</li>';
		...
		foreach ($alpha as $ch) {
		 		...
		 		if ($ch == $lastinitial) {
		     		...
		     		echo "<a href=\"$wwwroot/blocks/email_list/email/participants.php?id=$courseid&amp;group=$selgroup&amp;perpage=$perpage&amp;search=$search&amp;roleid=$roleid&amp;fname=$firstinitial&amp;lname=$ch')\">$ch</a>";
		 		}
		 		$nch++;
		}
	    ...
	    if (count($rolenames) > 1) {
	        ...
	        popup_form("$wwwroot/blocks/email_list/email/participants.php?id=$courseid&amp;group=$selgroup&amp;page=$page&amp;perpage=$perpage&amp;search=$search&amp;fname=$firstinitial&amp;lname=$lastinitial&amp;contextid=$context->id&amp;roleid=", $rolenames,
	                   'rolesform', $roleid, '');
	        echo '</div>';
	    }
