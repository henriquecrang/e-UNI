	...
	switch ( $action ) {
		...
		case md5('edit'):
			...
			if ( $data = $mform->get_data() ) {
				...
				if (! $oldfolder = get_record('block_email_list_folder', 'id', $data->id) ) {
					print_error('failgetfolder', 'block_email_list');
				}
				...
				if ( $subfolder = email_is_subfolder($oldfolder->id) ) {
					...
					if ( $subfolder->folderparentid != $data->parentfolder ) {
						if (! set_field('block_email_list_subfolder', 'folderparentid', $data->parentfolder, 'id', $subfolder->id) ) {
				...
				if ( $preference = get_record('block_email_list_preference', 'userid', $USER->id) ) {
						...
						if ( $oldfolder->course != $data->foldercourse ) {
							if ( $subfolders = email_get_all_subfolders($data->id) ) {
								foreach ($subfolders as $subfolder0) {
									set_field('block_email_list_folder', 'course', $data->foldercourse, 'id', $subfolder0->id);
								}
							}
						}
					}
				...
				// Update record
				if (! update_record('block_email_list_folder', $updatefolder) ) {
				    	return false;
				}
